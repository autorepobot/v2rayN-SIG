...
DOTNET_SDK_VERSION="${DOTNET_SDK_VERSION:-10.0.111}"
DOTNET_RUNTIME_VERSION="${DOTNET_RUNTIME_VERSION:-10.0.11}"
DOTNET_PPC64LE_PACKAGES="${HOME}/.nuget/ppc64le-runtime"

DOTNET_CORE_RUNTIME_PACKAGE="Microsoft.NETCore.App.Runtime.linux-ppc64le"
...
This script only supports: Debian."
  ...
  case "$HOST_ARCH" in
    x86_64|aarch64) ;;
    *) die "Only supports x86_64 / aarch64 hosts for ppc64le cross build" ;;
  esac
...
download_dotnet_runtime_pack() {
  ...
  url="https://github.com/IBM/dotnet-s390x/releases/download/v${DOTNET_SDK_VERSION}/${filename}"

  echo "[+] Download .NET runtime pack: $url"
  curl -fL "$url" -o "$outdir/$filename"
}

prepare_dotnet_ppc64le_runtime_packs() {
  local package=""

  mkdir -p "$DOTNET_PPC64LE_PACKAGES"

  for package in \
    "$DOTNET_ASPNET_RUNTIME_PACKAGE" \
    "$DOTNET_CORE_HOST_PACKAGE" \
    "$DOTNET_CORE_RUNTIME_PACKAGE"
  do
...
install_dependencies() {
  ...
  if command -v apt-get >/dev/null 2>&1; then
    ...
    sudo dpkg --add-architecture ppc64el || true
    sudo apt-get update
    sudo apt-get -y install \
      "libc6:ppc64el" \
      "libgcc-s1:ppc64el" \
...
bundle_url_for_rid() {
  ...
  case "$rid" in
    linux-ppc64le) echo "https://raw.githubusercontent.com/2dust/v2rayN-core-bin/refs/heads/master/v2rayN-linux-ppc64le.zip" ;;
    *)             return 1 ;;
  esac
}
...
describe_target() {
  ...
  case "$short" in
    ...
    *)       echo "Unknown arch '$short' (use ppc64le)" >&2; return 1 ;;
  esac
}
